...
void scores (){
    ...
    {
        while (fgets(chaine,TAILLE_MAX,tableau_des_scores) != NULL)  /* on lit le fichier tant qu'on ne re�oit pas d'erreur (NULL)*/  /* On lit au max Taille_max caract�res du fichier, on stocke le tout dans "chaine" */
        {
...
void Initialisation_jeu (ST_PARAM_JEU Param_jeu, ST_SNAKE *serpent, ST_POMME *pomme)
{
    ...
    printSnakeHead();
    /*serpent->pos[0].x=serpent->tete.x-1;
    serpent->pos[0].y=serpent->tete.y-1;
    for (p=serpent->taille-1; p>0; p--)
    {
        serpent->pos[p].x=serpent->pos[p-1].x-1;
        serpent->pos[p].y=serpent->pos[p-1].y;
    }*/
}
